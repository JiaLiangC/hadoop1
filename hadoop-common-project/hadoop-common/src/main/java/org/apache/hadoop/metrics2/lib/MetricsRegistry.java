...
public class MetricsRegistry {
  ...
  // 这货就是注解metrics 对象的容器，反射生成注解metrics 对象的容器
  /**
  ...
  //用户自定义的 metrics 一般在getMetrics 的时候会把所有metrics 调用调用recorder 的 builder.addGauge 方法加入 recorderbuilder中去
  //
  /**
  ...
  public synchronized void snapshot(MetricsRecordBuilder builder, boolean all) {
    ...
    for (MutableMetric metric : metrics()) {
      //MutableMetric.snapshot 的实现在具体的counter类中，比如MutableGaugeInt 中：builder.addGauge(info(), value);
