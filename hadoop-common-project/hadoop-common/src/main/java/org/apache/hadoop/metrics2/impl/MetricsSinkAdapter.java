...
class MetricsSinkAdapter implements SinkQueue.Consumer<MetricsBuffer> {
  ...
  // metricsSystem put buffer 数据过来，sinkAdapter 向 sinkQueue 入队数据
  // put 失败一般是满了，满了返回false 并且丢弃metrics 标记为失败
  // 这里入队操作是非阻塞的
  ...
  //要求入队的metrics 在超时时间内立刻被消费，否则就认为失败，增加 dropped metrics 计数
  public boolean putMetricsImmediate(MetricsBuffer buffer) {
    ...
    // 等待buffer 被消费，指定时间内没被消费就认为 put 失败
  ...
  //从队列消费 metrics，这里是 metricsSinkAdapter 新启动了一个线程来做这个消费，sinkadapter 本身是在 metricsSystem 线程中工作的
  ...
  // 这是对 SinkQueue consume 接口的实现，主要逻辑是消费 SinkQueue 的数据
  // 把数据出队，然后put 到这个 sinkAdapter 对应的 sink 中去，sink 通过自己的逻辑去输出这个metrics 数据
  @Override
