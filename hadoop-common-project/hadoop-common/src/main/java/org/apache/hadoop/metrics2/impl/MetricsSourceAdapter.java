...
// 一个 source 的适配器，主要适配了 jmx mbean source
class MetricsSourceAdapter implements DynamicMBean {
  ...
  // MetricsCollectorImpl 是 RecorderBuilder 的容器, 调用 MetricsCollectorImpl.addRecord 返回一个 RecordBuilder
  // 这里的逻辑是 1. medicsSystem 调用 getMetrics 获取sa 的 metricsRecords
  // 2. 因为 metricsSource 的逻辑是 传入MetricsCollector  然后用 collector.addRecord 会添加并返回一个metricsBuilder
  //3. 用户调用 builder 新建metrics
  //4. get metrics 的时候，迭代 MetricsCollectorImpl ，收集所有的metricsRecord 返回
  Iterable<MetricsRecordImpl> getMetrics(MetricsCollectorImpl builder,
    ...
    synchronized(this) {
      ...
      //这里返回的是MetricRecordImpl 的 一个list。 Iterable->Collection->List
      // 这里大量使用 Iterable 应该是为了提供迭代功能，但是不提供collection的修改功能，作为 immutable object的一种实现。
      // MetricsCollectorImpl 作为流水线装配工，使用List 允许添加删除，但是出了 MetricsCollectorImpl 就全是 Iterable 对象，不可修改的不可变容器
      return lastRecs;
    }
  }
